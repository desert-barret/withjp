<template>
  <div>
    ...
    <div class="flex items-start sm:items-center justify-between gap-4 mb-6 sm:mb-8">
      <div>
        <h1 class="font-display font-bold text-xl sm:text-2xl text-slate-900 dark:text-white">Academia</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-0.5 text-sm">
          {{ items.length }} curso{{ items.length !== 1 ? 's' : '' }} registrado{{ items.length !== 1 ? 's' : '' }}
        </p>
      </div>
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-slate-200/80 dark:border-white/[0.06] bg-slate-50/50 dark:bg-white/[0.01]">
              <th class="text-left px-5 py-3.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Curso</th>
              <th class="text-left px-5 py-3.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider hidden md:table-cell">Subdominio</th>
              <th class="text-left px-5 py-3.5 text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider hidden lg:table-cell">Estado</th>
            ...
            <tr v-for="item in items" :key="item.id"
              ...
              <td class="px-5 py-4 hidden md:table-cell">
                <span v-if="item.subdomain_url"
                  class="text-xs text-violet-600 dark:text-violet-400 font-mono truncate max-w-[180px] block">
                  {{ item.subdomain_url }}
                </span>
              ...
              <td class="px-5 py-4 hidden lg:table-cell">
                <div class="flex items-center gap-2">
                  <span class="flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full"
                    ...
                    <span class="w-1.5 h-1.5 rounded-full" :class="item.active ? 'bg-emerald-500' : 'bg-slate-400'" />
            ...
            <tr v-if="items.length === 0">
              <td colspan="4" class="px-6 py-16 text-center">
                <div class="flex flex-col items-center gap-3">
                  <div class="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-white/[0.05] flex items-center justify-center text-2xl">🎓</div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div class="relative w-full sm:max-w-2xl max-h-[92vh] overflow-y-auto
            ...
            <div class="sticky top-0 z-10 flex items-center justify-between px-5 sm:px-6 py-4
                        bg-white dark:bg-[#0d1117] border-b border-slate-200/80 dark:border-white/[0.07]">
            ...
            <form @submit.prevent="handleSave" class="p-5 sm:p-6 space-y-6">
              ...
              <div class="pt-4 border-t border-slate-200/80 dark:border-white/[0.07] space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label class="label">URL del subdominio</label>
                    <input v-model="form.subdomain_url" type="text" class="input" placeholder="https://claude.withjp.ai" />
                  </div>
                  <div>
                    <label class="label">Badge (ej. NUEVO, BETA)</label>
                    <input v-model="form.badge" type="text" class="input" placeholder="NUEVO" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text" class="input" placeholder="Claude Code, AI, Shell..." />
                </div>
              ...
              <div class="flex gap-3 pt-1">
                ...
                <button type="button" @click="closeModal" class="btn-secondary px-5 py-2.5 text-sm">Cancelar</button>
              </div>
            </form>
          </div>
        </div>
...
const catIcon = (cat: string) => ({ ai:'🧠', web:'🌐', automation:'🤖', mobile:'📱' }[cat] || '🎓');

function getTitle(item: any, locale: string) {
  return item?.translations?.find((t: any) => t.locale === locale)?.title || '—';
}

function emptyForm() {
  return {
    id: null as number | null,
    subdomain_url: '', image_url: '', badge: '',
...
function openModal(item?: any) {
  ...
  if (item) {
    const tEs = item.translations?.find((t: any) => t.locale === 'es') || {};
    const tEn = item.translations?.find((t: any) => t.locale === 'en') || {};
    form.value = {
      id: item.id,
...
async function handleSave() {
  ...
  try {
    const payload = {
      subdomain_url: form.value.subdomain_url || undefined,
    ...
    if (form.value.id) {
      await academiaApi.update(form.value.id, payload);
    } else {
...
async function handleDelete() {
  ...
  saving.value = true;
  try {
    await academiaApi.remove(deleteTarget.value.id);
    await loadItems();
...
async function loadItems() {
  loading.value = true;
  try {
    const { data } = await academiaApi.getAll(true);
    items.value = data;
  } finally {
    loading.value = false;
  }
